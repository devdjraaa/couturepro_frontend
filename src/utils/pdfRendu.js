...
export async function partagerOuTelecharger(pdf, nomFichier, { title = 'Document', text = '' } = {}) {
  // BRANCHE ANDROID — point de délégation UNIQUE au partage natif.
  //
  // `navigator.share` est peu fiable pour un fichier dans une WebView
  // Capacitor : selon l'appareil il ignore la pièce jointe ou échoue en
  // silence. Le plugin natif (Filesystem + Share) écrit le PDF dans le cache et
  // passe un URI `content://` que les autres applications savent ouvrir.
  //
  // Concentrer cette différence ICI plutôt que dans les huit exports est
  // délibéré : c'est ce qui permet à cette branche de recevoir toute la charte
  // PDF de `master` sans que huit fichiers se remettent à diverger.
  if (Capacitor.isNativePlatform()) {
    await shareOrSavePdf(pdf, nomFichier)
    return true
  }
